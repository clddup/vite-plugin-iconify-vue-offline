...
const iconJSON = Object.fromEntries(Object.keys(icons).map(item => [item, {path: item, json: undefined}]))
    ...
    async load(id) {
      ...
      if (isBuild && id === resolvedVirtualModuleId) {
        ...
        for (let i in iconObj) {
          if (iconJSON[i]) {
            if (!iconJSON[i].json) {
              iconJSON[i].json = await lookupCollection(i)
            }
            iconJSON[i].useJSON = vf.getIcons(iconJSON[i].json, iconObj[i])
          }
        }
